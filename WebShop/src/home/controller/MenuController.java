...
//	}
	...
	public String signIn(ModelMap model, @ModelAttribute("user") User user, BindingResult errors) {
		...
		if (baseURL.equals("order")) {
			...
			if(tempUser.getRoleId()==1) { return "redirect:/admin/user/1.html"; }
...
//	}
	...
	public int totalAllItem() {
		...
		try {
			...
			Long obj = (Long) query.uniqueResult();
			t.commit();
			return obj.intValue();// ép kiểu sang int
		} catch (Exception ex) {
			if (t != null) {
				System.out.println(ex.toString());
				t.rollback();
	...
	public String insertCategory(ModelMap model, @ModelAttribute("category") Category category,
		...
		try {
			...
			if (!photo.isEmpty()) {
				...
				//String photoPath = "D:\\Java WEB\\project\\WebShop\\WebContent\\images\\category-image\\" + photo.getOriginalFilename();
				photo.transferTo(new File(photoPath));
	...
	public String deleteUser(ModelMap model, @PathVariable("id") Integer id) {
		...
		//Query query = session.createQuery("FROM Carts as carts WHERE carts.UserId="+ id);
		//List<Cart> list = query.list();
		//int i=0;
